...
<section id="list">
	...
	<?php foreach ($lists as $list) {
		...
		<div class="list" list="<?= $list->id() ?>" progress="<?= $list->progressBar() ?>">
			...
			<?php } ?>
			<p class="grey invisible"><span class="edit">Modifier</span> - <span class="delete">Supprimer</span></p>
			<p class="list-description invisible"><?= nl2br($list->description()) ?></p>
			<?php if(count($tasksList)) { ?>
				...
				<div class="addtask-list invisible"><i class="fas fa-plus fa-fw"></i> Ajouter une tâche</div>
			<?php } else { ?>
				<p class="invisible">Pas de tâche.</p>
				<div class="addtask-list invisible"><i class="fas fa-plus fa-fw"></i> Ajouter une tâche</div>
			<?php } ?>
		</div>
	<?php } ?>
